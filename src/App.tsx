      ...
      <div className="relative z-10 flex flex-col h-full p-10 max-w-[1400px] mx-auto w-full">
        ...
        <div className="grid grid-cols-[280px_1fr] gap-10 grow min-h-0">
          <aside className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              <span className="text-[11px] uppercase tracking-[0.1em] text-[var(--text-muted)] mb-1 font-semibold">Components</span>
              <button 
              ...
              <button 
                ...
                className={`px-4 py-3 rounded-[10px] text-[14px] text-left transition-all ${activePage === 'Modals' ? 'bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-main)] backdrop-blur-[10px]' : 'border border-transparent text-[var(--text-muted)] hover:bg-[var(--secondary-btn-hover)]'}`}>Modals</button>
              <button 
                onClick={() => setActivePage('Cards')}
                className={`px-4 py-3 rounded-[10px] text-[14px] text-left transition-all ${activePage === 'Cards' ? 'bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-main)] backdrop-blur-[10px]' : 'border border-transparent text-[var(--text-muted)] hover:bg-[var(--secondary-btn-hover)]'}`}>Cards</button>
              <button 
                onClick={() => setActivePage('Overlays')}
                className={`px-4 py-3 rounded-[10px] text-[14px] text-left transition-all ${activePage === 'Overlays' ? 'bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-main)] backdrop-blur-[10px]' : 'border border-transparent text-[var(--text-muted)] hover:bg-[var(--secondary-btn-hover)]'}`}>Overlays</button>
            </div>
          </aside>
          ...
          <main className="flex flex-col gap-6 relative overflow-y-auto pb-20">
            ...
            {activePage === 'Modals' && (
              ...
              {/* Code Block Snippet */}
              <div className="bg-[var(--code-bg)] rounded-xl p-5 font-mono text-[13px] leading-[1.6] text-indigo-400 border border-[var(--code-border)] animate-in fade-in slide-in-from-bottom-2">
